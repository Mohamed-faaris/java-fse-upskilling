 ...
 * HQL stands for Hibernate Query Language, JPQL stands for Java Persistence Query Language, Compare
 * HQL and JPQL, @Query annotation, HQL fetch keyword, aggregate functions in HQL, Native Query,
 ...
 *
 * Hands on 2
 * Get all permanent employees using HQL Using HQL get all permanent employees. When retrieving the
 * employee details it should also retrieve respective department and skill list as well.HQL Solution
 ...
 * Include a new test method and Invoke the service method in OrmLearnApplication.java. Refer test
 * method below that logs all employee details and each employee's skill details.
 ...
 * Employee.java
 * Remove these two eager fetch configurations and check the logs. The following queries would have got
 * executed. It would have failed when getting the skill list. Since we have remove the eager fetch
 * skill is not retrieved.
 ...
 * @Query(value="SELECT e FROM Employee e left join e.department d left join e.skillList WHERE
 * e.permanent = 1")
 ...
 * specified below:
 * @Query(value="SELECT e FROM Employee e left join fetch e.department d left join fetch e.skillList
 * WHERE e.permanent = 1")
 ...
 * skilllist2_.es_sk_id=skill3_.sk_id
 * where  employee0_.em_permanent=1
 * IMPORTANT TAKEAWAY: Join keyword links the table, but does not populate the beans. Fetch ensures
 * that the beans are populated. Based on our need wherever we need data, we can define fetch. When
 * joining table data is not needed the fetch can be ignored.
 *
 * Hands on 3
 ...
 *
 * Hands on 4
 * Get average salary using HQL Compute the average salary of a department using HQL.Refer steps below
 * to implement:
 ...
 * product?The where clause varies based on the criteria selected by the user. We have to dynamically
 * frame the where clause filters based on the criteria selected by user.Criteria Query helps in
 * handling this scenario in a better way. The filter criteria can be programmatically added, rather
 * than fixing the HQL Statement.Go through the examples in the link below to understand how Criteria
 * Query has to be implemented.https://howtodoinjava.com/hibernate/hibernate-criteria-queries-tutorial/
 */
public class Main {
