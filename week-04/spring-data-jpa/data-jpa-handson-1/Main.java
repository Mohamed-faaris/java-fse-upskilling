 ...
 * ORM (Object-Relational Mapping), makes it easier to develop code that interacts with database,
 ...
 * Evolution of ORM solutions, Hibernate XML Configuration, Hibernate Annotation Configuration, Spring
 * Data JPA, Hibernate benefits, open source, light weight, database independent query
 ...
 * Hibernate Architecture Reference -
 ...
 * XML Configuration - persistence class, mapping xml, configuration xml, loading hibernate
 * configuration xml file; Annotation Configuration - persistence class, @Entity, @Table, @Id, @Column,
 * hibernate configuration xml file Loading hibernate configuration and interacting with database get
 * the session factory, open session, begin transaction, commit transaction, close session
 * XML Configuration Example - https://www.tutorialspoint.com/hibernate/hibernate_examples.htm
 * Hibernate Configuration Example - https://www.tutorialspoint.com/hibernate/hibernate_annotations.htm
 *
 * Explain the difference between Java Persistence API, Hibernate and Spring Data JPA
 * JPA (Java Persistence API), JPA is a specification (JSR 338), JPA does not have implementation,
 * Hibernate is one of the implementation for JPA, Hibernate is a ORM tool, Spring Data JPA is an
 * abstraction above Hibernate to remove boiler plate code when persisting data using Hibernate.
 * Difference between Spring Data JPA and Hibernate - https://dzone.com/articles/what-is-the-
 * difference-between-hibernate-and-sprin-1
 ...
 * Hibernate log configuration and ddl-auto configuration, JpaRepsitory.findById(), defining Query
 * Methods, JpaRespository.save(), JpaRepository.deleteById()
 * Spring Data JPA Ref Repository methods - https://docs.spring.io/spring-
 * data/jpa/docs/2.2.0.RELEASE/reference/html/#repositories.core-concepts
 * Query methods - https://docs.spring.io/spring-
 * data/jpa/docs/2.2.0.RELEASE/reference/html/#repositories.query-methods
 *
 * Hands on 1
 ...
 * Import the project in Eclipse "File > Import > Maven > Existing Maven Projects > Click Browse and
 * select extracted folder > Finish"
 * Create a new schema "ormlearn" in MySQL database. Execute the following commands to open MySQL
 * client and create schema.
 * > mysql -u root -p
 *
 * mysql> create schema ormlearn;
 * In orm-learn Eclipse project, open src/main/resources/application.properties and include the below
 ...
 * Create a new table country with columns for code and name. For sample, let us insert one country
 * with values 'IN' and 'India' in this table.
 ...
 *
 * Hands on 2
 * Hibernate XML Config implementation walk through SME to provide explanation on the sample Hibernate
 * implementation available in the link
 * below:https://www.tutorialspoint.com/hibernate/hibernate_examples.htmExplanation Topics
 ...
 *
 * Hands on 3
 * Hibernate Annotation Config implementation walk through SME to provide explanation on the sample
 * Hibernate implementation available in the link
 * below:https://www.tutorialspoint.com/hibernate/hibernate_annotations.htmExplanation Topics
 ...
 * }
 * ​​​​​​​ Reference Links: https://dzone.com/articles/what-is-the-difference-between-hibernate-and-
 * sprin-1 https://www.javaworld.com/article/3379043/what-is-jpa-introduction-to-the-java-persistence-
 * api.html
 *
 * Hands on 5
 * Implement services for managing Country An application requires for features to be implemented with
 * regards to country. These features needs to be supported by implementing them as service using
 * Spring Data JPA.
 ...
 * Before starting the implementation of the above features, there are few configuration and data
 * population that needs to be incorporated. Please refer each topic below and implement the
 * same. Explanation for Hibernate table creation configuration
 * Moreover the ddl-auto defines how hibernate behaves if a specific table or column is not present in
 * the database.
 * create - drops existing tables data and structure, then creates new tables
 * validate - check if the table and columns exist or not, throws an exception if a matching table or
 * column is not found
 * update - if a table does not exists, it creates a new table; if a column does not exists, it creates
 * a new column
 * create-drop - creates the table, once all operations are completed, the table is dropped
 * # Hibernate ddl auto (create, create-drop, update, validate)
 * spring.jpa.hibernate.ddl-auto=validate
 * Populate country table
 * Delete all the records in Country table and then use the below script to create the actual list of
 * all countries in our world.
 ...
 * insert into country (co_code, co_name) values ("SH", "Saint Helena, Ascension and Tristan da
 * Cunha");
 ...
 * insert into country (co_code, co_name) values ("GS", "South Georgia and the South Sandwich
 * Islands");
 ...
 * Include new test method in OrmLearnApplication to find a country based on country code and compare
 * the country name to check if it is valid.
 ...
 * Create a new method updateCountry() in CountryService with parameters code and name. Annotate this
 * method with @Transactional. Implement following steps in this method.
