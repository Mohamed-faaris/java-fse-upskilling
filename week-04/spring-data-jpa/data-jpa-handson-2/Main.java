 ...
 * Query Methods - Search by containing text, sorting, filter with starting text, fetch between dates,
 ...
 * @ManyToOne, @JoinColumn, @OneToMany, FetchType.EAGER, FetchType.LAZY, @ManyToMany, @JoinTable,
 * mappedBy
 * Relationships reference - https://www.baeldung.com/spring-data-rest-relationships
 *
 * Hands on 1
 * Write queries on country table using Query Methods Following are the list of queries that is
 * required for an application. Implement these queries using Query Methods feature of Spring Data JPA.
 * Click here for reference. Include appropriate methods in OrmLearnApplication and test the same.
 * An application has a search text box for searching by country. When typing characters on the text
 * box, a list of all the matching countries should be displayed. For example, if 'ou' is entered in
 * the search box the following countries should be displayed. Write a Query Method to achieve this
 ...
 * ZA       South Africa
 * Enhance the above method to return the countries in ascending order. Modify the query method name
 * defined in the previous problem to achieve this.
 * BV       Bouvet Island
 * DJ       Djibouti
 * TF       French Southern Territories
 * GP       Guadeloupe
 ...
 *
 * Hands on 2
 * Write queries on stock table using Query Methods With one year stock data of Facebook, Google and
 * Netflix, we need to implement Spring Data JPA Query Methods for the following scenarios:Sample
 * DataSample data for implementing this hands on is provided to you in the platform
 * Setup stock data
 ...
 * and Netflix from 18 Oct 2018 to 17 Oct 2019. This is public data downloaded from finance.yahoo.com.
 ...
 * Copy all data in F column and paste it in Notepad or Notepad++ and save it as a file named stock-
 * data.sql. Execute this script in mysql command line client which populates data into
 ...
 *
 * Hands on 3
 * Create payroll tables and bean mapping To demonstrate one to many, many to one and many to many
 * relationships in Hibernate, a schema with entities employee, department and skill will be used. In
 * this hands on we will setup the tables and data, which forms the basis for learning the mappings in
 * Hibernate.Schema StructureFollow steps below to create necessary tables:
 ...
 * Each id field should be have @Id annotation and @GeneratedValue(strategy = GenerationType.IDENTITY)
 * annotation. @GeneratedValue annotation ensures auto increment of id creation.
 ...
 *
 * Hands on 4
 * Implement many to one relationship between Employee and Department Follow steps below to defined
 * many to one relationship and perform persistence operations:Preparation of Service Classes
 * Create EmployeeService, DepartmentService and SkillService defined with annotation @Service. In each
 * of this class autowire respective repository.
 * In each of the service class implement two methods one is to get the entity based on id and the
 * other one is to save the entity. Sample code below provided for EmployeeService, in similar fashion
 * include the methods for DepartmentService and SkillService.
 ...
 * OrmLearnApplication.
 * Assign employeeService, departmentService and skillService from the context in OrmLearnApplication
 * main() method.
 * Implementation of @ManyToOne mapping
 * Define department in Employee bean with @ManyToOne and @JoinTable annotation. This defines the
 ...
 * Since the relationship is defined, hibernate fetches department data as well. The query should look
 * something like the below. Observe the department table join in this query. The query is formatted
 ...
 * as well is joined and fetched by Hibernate. Add Employee
 ...
 * Observe that the employee log after save contains the id. Hibernate inserts the records, gets the id
 * and set the id instance variable of employee
 ...
 * Get a department based on department id using departmentService. Use a different department id from
 ...
 * Invoke the main method and check the following:
 * Log should contain select query to get the department and employee. There should be update statement
 * that updates employee table
 ...
 * In this method, get a department using departmentService.get() passing the id. Select an department
 * id that has more than one employee.
 ...
 * default fetch type for OneToMany relationship is LAZY, hibernate fetches only department details and
 * does not get the employee details.
 * In order to get the employee list as well, modify the annotation to include the fetch type as EAGER.
 * Make this change in employeeList annotation definition of Department class.
 ...
 * In testGetEmployee() method of OrmLearnApplication.java include a new line to log employee skill
 * details after the department log.
 * LOGGER.debug("Skills:{}", employee.getSkillList());
 * Execute the main method which will fail with LazyInitializationException
 * Include fetch type as eager in @ManyToMany annotation of Employee.java, which will fetch the skill
 * details as well.
