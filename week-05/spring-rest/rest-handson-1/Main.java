 ...
 * Makes Java development easy, avoids tedious development steps, reduces development time, avoids
 * writing boilerplate code, provides embedded tomcat server, avoid XML configuration
 * Ref - https://www.journaldev.com/7969/spring-boot-tutorial
 * Demonstrate loading bean from spring configuration file
 * Spring configuration xml, spring xml schema spring-beans.xsd, <bean>, id, class, <constructor-arg>,
 * <property>, name, value, ClassPathXmlApplicationContext, ApplicationContext, context.getBean(),
 * singleton scope, prototype scope
 * Ref - https://docs.spring.io/spring-framework/docs/current/spring-framework-reference/core.html
 * IoC Container - https://docs.spring.io/spring-framework/docs/current/spring-framework-
 * reference/core.html#beans
 * Scopes - https://docs.spring.io/spring-framework/docs/current/spring-framework-
 * reference/core.html#beans-factory-scopes
 * Constructor Injection - https://docs.spring.io/spring-framework/docs/current/spring-framework-
 * reference/core.html#beans-constructor-injection
 * Setter method injection - https://docs.spring.io/spring-framework/docs/current/spring-framework-
 * reference/core.html#beans-setter-injection
 * Demonstrate inclusion of logging in Spring Boot Application
 * application.properties, logging.level, logging.pattern, server.port, LoggerFactory, Logger, log
 ...
 *
 * Hands on 2
 * Spring Core – Load SimpleDateFormat from Spring Configuration XML SimpleDateFormat with the pattern
 * ‘dd/MM/yyyy’ is created in multiple places of an application. To avoid creation of SimpleDateFormat
 * in multiple places, define a bean in Spring XML Configuration file and retrieve the date.Follow
 * steps below to implement:
 * Create spring configuration file date-format.xml in src/main/resources folder of 'spring-learn'
 * project
 * Open https://docs.spring.io/spring-framework/docs/current/spring-framework-
 * reference/core.html#beans-factory-metadata
 ...
 * Using the format variable try to parse string '31/12/2018' to Date class and display the result
 * using System.out.println.
 * Run the application as 'Java Application' and check the result in console log output.
 * Troubleshooting Tips If the tomcat port has a conflict and the server is not starting include the
 * below property in application.properties file in src/main/resources folder.
 *
 * Hands on 3
 * Spring Core - Incorporate Logging Incorporate logging in the Spring Boot project created in previous
 * hands on. Refer steps below:
 ...
 * LOGGER.info(“END”);
 * }
 * IMPORTANT NOTE: Going forward all methods should incorporate logging as specified above. Never use
 * System.out.println().
 * Hands on 4
 * Spring Core – Load Country from Spring Configuration XML An airlines website is going to support
 * booking on four countries. There will be a drop down on the home page of this website to select the
 * respective country. It is also important to store the two-character ISO code of each country.
 * Code
 * Name
 * US
 * United States
 ...
 * JP
 * Japan
 * Above data has to be stored in spring configuration file. Write a program to read this configuration
 * file and display the details.Steps to implement
 ...
 * Implement empty parameter constructor with inclusion of debug log within the constructor with log
 * message as “Inside Country Constructor.”
 * Generate getters and setters with inclusion of debug with relevant message within each setter and
 * getter method.
 * Generate toString() method
 * Create a method displayCountry() in SpringLearnApplication.java, which will read the country bean
 * from spring configuration file and display the country
 * details. ClassPathXmlApplicationContext, ApplicationContext and context.getBean(“beanId”,
 * Country.class). Refer sample code for displayCountry() method below.
 ...
 *
 * Hands on 5
 * Spring Core – Demonstration of Singleton Scope and Prototype Scope The Country bean done in the
 * previous hands on will be used to demonstrate the scopes in Spring. Implement the steps below.Follow
 * steps below to demonstrate Singleton Scope
 * Include a line in displayCountry() to get country bean reference one more time from the same
 * application context. Only the third line of the below code snippet should be copied and pasted.
 * ApplicationContext context = new ClassPathXmlApplicationContext("country.xml");
 * Country country = context.getBean("country", Country.class);
 * Country anotherCountry = context.getBean("country", Country.class);
 * The constructor will be called only once, which means that only one instance of Country bean is
 * created
 ...
 *
 * Hands on 6
 * Spring Core – Load list of countries from Spring Configuration XML Our main objective was to
 * retrieve the list of four countries for the airlines website. Refer steps below to get this
 * incorporated.
