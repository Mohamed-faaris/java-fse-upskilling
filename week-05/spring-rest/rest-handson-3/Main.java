 ...
 * REST Web Service architecture with Controller, Service and Dao, service methods.
 * NOTE: There is no Quiz for this session
 *
 * Problem Statement - Display Employee List and Edit Employee form using RESTful Web Service In the
 * previous angular module, we developed a screen that lists employees and it was populated with hard
 * coded values. Now this angular application has be changed to get the data from RESTful Web Service
 * developed in Spring. The following are the high level activities that needs to be done to accomplish
 * this:
 ...
 * Once above activities are completed, clicking on the Edit button against each employee should
 * display Edit Employee form with values retrieved from RESTful Web Service. This will also involve
 * activities similar to the one specified above.NOTE: There is no specific activity as part of this
 * hands on, refer the next hands ons that covers above three activities in detail.
 *
 * Create static employee list data using spring xml configuration Follow steps below to accomplish
 * this activity:
 ...
 * Create method getAllEmployees() that invokes employeeDao.getAllEmployees() and return the employee
 * list
 ...
 * getAllDepartments()
 * DepartmentDao
 * getAllDepartments() - Create a static variable DEPARTMENT_LIST, this should be populated from spring
 * xml configuration
 *
 * Test ​the service using postman.
