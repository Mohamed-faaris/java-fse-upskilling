 ...
 *
 * Developers of Apps Centric Solutions have created an employee management application which supports
 * light and dark themes for the buttons. The current solution uses the react state and props to
 * provide the theme name to be used from App component to Employee List component and from there to
 * Employee Card component. Quality assurance team analyzed the solutions and found the technique being
 * used to be a substandard one. React architect suggested to use the react context API to share the
 * theme name with nested child components instead of passing them down using props from the parent
 * component.
 ...
 * Create a new file with the name as ThemeContext.js. Define a new context in the file with the name
 * as ThemeContext and assign it a default value of ‘light’ and export it as default form the module.
 ...
 * Go to EmployeeList component present in EmployeeList.js file and modify it so that theme name is not
 * passed explicitly to its child component.
