 ...
 * Implement conflict resolution when multiple users are updating the trunk (or master) in such a way
 * that it results into a conflict with the branch’s modification.
 * Prerequisites
 *
 ...
 * Please follow the instructions to complete the hands-on. Each instruction expect a command for the
 * Git Bash.
 * Verify if master is in clean state.
 * Create a branch “GitWork”. Add a file “hello.xml”.
 * Update the content of “hello.xml” and observe the status
 * Commit the changes to reflect in the branch
 * Switch to master.
 * Add a file “hello.xml” to the master and add some different content than previous.
 ...
 * Merge the bran to the master
 * Observe the git mark up.
